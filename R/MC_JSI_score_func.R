...
MC_JSI_score_func<- function(data_s1_sub = data_s1_sub,
                             tf_left_1 = tf_left_1,
  ...
  tf_s_sample <- data_s1_sub$scores[which(data_s1_sub$genes==select_tf)]
  ave_s_sample <- mean(sample_data[,2])
  
  #Calculating the module specificity score
  regulon_s_sample <- tf_s_sample + theta*ave_s_sample
